    ...
    <div className="flex h-screen w-full bg-[#F8FAF9] font-sans overflow-hidden text-slate-800">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-6 md:px-8 flex-shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg font-bold text-slate-900">
              {activeTab === 'My Profile' ? 'Shelter Profile' : 'Shelter Management'}
            </h2>
          </div>
              ...
              {notifOpen && (
                <div className="absolute right-0 top-12 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                  ...
                  <div className="divide-y divide-slate-50">
                    {[
                      { text: 'New adoption application submitted for Bruno (Labrador)', time: '20 min ago' },
                      { text: 'Cage D-04 status updated to Occupied', time: '2h ago' },
                      { text: 'Monthly shelter capacity audit verified', time: '1d ago' },
                    ].map((n, i) => (
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
          ...
          {activeTab === 'Shelter Dashboard' && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <button 
                    onClick={() => setActiveTab('Manage Adoptions')}
            ...
            <div className="max-w-3xl space-y-6">
              ...
              <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <h2 className="text-xl font-black text-slate-900">{user?.fullName || 'Green Valley Facility'}</h2>
                      <span className="px-3 py-1 bg-amber-50 text-amber-700 border border-amber-200/60 rounded-xl text-xs font-bold">
                        Shelter Manager
                      </span>
                    </div>
                    <p className="text-xs font-semibold text-slate-500">{user?.email || 'contact@greenvalleyshelter.org'}</p>
                    <p className="text-xs text-slate-400 font-medium flex items-center gap-1.5 pt-0.5">
                      <MapPin className="w-3.5 h-3.5 text-slate-400" /> Sector 22, Green Valley Enclave, Bangalore
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t border-slate-100">
                  {[
                    { label: 'Shelter Facility ID', value: 'S01' },
                    { label: 'Registration Type', value: 'State Trust / Society' },
                    { label: 'Phone Contact', value: user?.phoneNumber || '+91 94471 23456' },
                    { label: 'Total Capacity', value: '45 Cages (18 Available)' },
                    { label: 'Active Staff', value: '12 Full-time caretakers' },
                    { label: 'Facility Status', value: 'OPEN (Accepting Animals)' },
                  ].map((field) => (
          ...
          {/* Fallback tabs linked to existing User views for consistency */}
          {activeTab !== 'Shelter Dashboard' && activeTab !== 'My Profile' && (
